...
const TvSort = () => {
  ...
  const { tvByPopularity, tvByReleaseDate, tvByTitleDesc, tvByTitleAsc } =
    useSelector((state) => state.filteredTv);

  const handleSort = (event) => {
    const id = event.target.value;
    switch (id) {
      case "0":
        return dispatch(sortTv(tvByPopularity));
      case "1":
        return dispatch(sortTv(tvByReleaseDate));
      case "2":
        return dispatch(sortTv(tvByTitleDesc));
      case "3":
        return dispatch(sortTv(tvByTitleAsc));
      default:
        return dispatch(sortTv(tvByPopularity));
    }
  };
